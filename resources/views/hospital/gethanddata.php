...
if(isset($_POST['request_no'])){
	...
	$qr				= "SELECT * FROM requests
						left join blood_group on blood_group.id=requests.blood_group_id 
						WHERE requests.ref_code='$request_no'";
	$result 		= mysqli_query($conn,$qr) or die(mysqli_error($conn)); 
	...
	while($datares = mysqli_fetch_array($result)){	
		...
		$hand = array(
				...
				"blood_group"	=>	$datares['blood_group_id'],
				"phone"			=>	$datares['phone'],
